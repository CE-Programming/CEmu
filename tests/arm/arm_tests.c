...
static void test_compare_and_branch(void) {
    ...
    prepare_instruction(&arm, UINT16_C(0xB110)); /* CBZ r0, +4 */
    arm.cpu.r0 = 0;
    arm.cpu.n = true;
    arm.cpu.z = false;
    arm.cpu.c = true;
    arm.cpu.v = true;
    arm_cpu_execute(&arm);
    CHECK(arm.cpu.pc == UINT32_C(0x8), "CBZ branches when its register is zero");
    CHECK(arm.cycles == 2, "taken CBZ takes two cycles");
    CHECK(arm.cpu.n && !arm.cpu.z && arm.cpu.c && arm.cpu.v,
          "CBZ preserves condition flags");

    prepare_instruction(&arm, UINT16_C(0xB110)); /* CBZ r0, +4 */
    arm.cpu.r0 = 1;
    arm_cpu_execute(&arm);
    CHECK(arm.cpu.pc == UINT32_C(0x4), "CBZ falls through when its register is nonzero");
    CHECK(arm.cycles == 1, "untaken CBZ takes one cycle");

    prepare_instruction(&arm, UINT16_C(0xB910)); /* CBNZ r0, +4 */
    arm.cpu.r0 = 1;
    arm_cpu_execute(&arm);
    CHECK(arm.cpu.pc == UINT32_C(0x8), "CBNZ branches when its register is nonzero");

    prepare_instruction(&arm, UINT16_C(0xB910)); /* CBNZ r0, +4 */
    arm.cpu.r0 = 0;
    arm_cpu_execute(&arm);
    CHECK(arm.cpu.pc == UINT32_C(0x4), "CBNZ falls through when its register is zero");

    prepare_instruction(&arm, UINT16_C(0xBB80)); /* CBNZ r0, +96 */
    arm.cpu.r0 = 1;
    arm_cpu_execute(&arm);
    CHECK(arm.cpu.pc == UINT32_C(0x64),
          "CBNZ decodes the high branch-offset bit");
