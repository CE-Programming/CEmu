...
void emu_loop(bool reset) {
    ...
    // Tell the Nspire's LCD controller to map its LCDUPBASE to the CE's RAM...
    uint32_t nspire_lcd_base_backup = (*(uint32_t*)0xC0000010);

    (*(uint32_t*)0xC0000010) = (uint32_t)((asic.mem->ram.block + 0x40000));

    t_key key_esc = KEY_NSPIRE_ESC;

    t_key key_1 = KEY_NSPIRE_1;
    t_key key_2 = KEY_NSPIRE_2;
    t_key key_enter = KEY_NSPIRE_ENTER;
    t_key key_plus = KEY_NSPIRE_PLUS;
    ...
    while (!isKeyPressed(key_esc)) {
        ...
        keypad_key_event(3, 1, isKeyPressed(key_1));
        keypad_key_event(4, 1, isKeyPressed(key_2));
        keypad_key_event(6, 0, isKeyPressed(key_enter));
        keypad_key_event(6, 1, isKeyPressed(key_plus));
    }
