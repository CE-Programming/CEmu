...
namespace tivars
{
    ...
    data_t TH_0x0D::makeDataFromString(const string& str, const options_t options)
    {
        ...
        {
            std::cerr << "Invalid input string. Needs to be a valid complex list" << endl;
            return data;
        }
    ...
    string TH_0x0D::makeStringFromData(const data_t& data, const options_t options)
    {
        ...
        {
            std::cerr << "Invalid data array. Needs to contain 2+" + to_string(TH_0x0C::dataByteCount) + "*n bytes" << endl;
            return "";
        }
