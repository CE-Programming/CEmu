...
void gui_console_err_printf(const char *format, ...) {
    va_list args;
    va_start(args, format);
    emu->writeConsole(CoreThread::ConsoleErr, format, args);
    va_end(args);
}
...
void CoreThread::writeConsole(int console, const char *format, va_list args) {
    if (type != console) {
...
void CoreThread::block(int status) {
    std::unique_lock<std::mutex> lock(m_mutex);
    emit blocked(status);
    m_cv.wait(lock);
}

void CoreThread::unblock() {
    m_mutex.lock();
    m_cv.notify_all();
    m_mutex.unlock();
}

asic_rev_t CoreThread::handleReset(const boot_ver_t* bootVer, asic_rev_t loadedRev, asic_rev_t defaultRev, bool* python) {
...
void CoreThread::reset() {
    std::lock_guard<std::mutex> requestLock{m_requestMutex};
    m_requestQueue.enqueue(RequestReset);
}

void CoreThread::cancelTransfer() {
    usbPlugDevice();
}

void CoreThread::receive() {
    std::lock_guard<std::mutex> requestLock{m_requestMutex};
    m_requestQueue.enqueue(RequestReceive);
}

void CoreThread::send(const QStringList &list, int location) {
    std::lock_guard<std::mutex> requestLock{m_requestMutex};
    m_vars = list;
    m_sendLoc = location;
...
void CoreThread::usbPlugDevice(const QStringList &args,
...
void CoreThread::doUsbPlugDevice() {
    std::unique_lock<std::mutex> requestLock{m_requestMutex};
...
void CoreThread::enqueueKeys(quint16 key1, quint16 key2, bool repeat) {
    QMutexLocker locker(&m_keyQueueMutex);
...
void CoreThread::test(const QString &config, bool run) {
    std::lock_guard<std::mutex> requestLock{m_requestMutex};
...
void CoreThread::save(emu_data_t fileType, const QString &filePath) {
    std::lock_guard<std::mutex> requestLock{m_requestMutex};
    m_savePath = filePath;
    m_saveType = fileType;
...
void CoreThread::setAsicRev(int rev) {
    m_asicRev.store(rev);
}

void CoreThread::setAllowAnyRev(bool allow) {
    m_allowAnyRev.store(allow);
}

void CoreThread::setForcePython(Qt::CheckState state) {
    m_forcePython.store(state);
}

void CoreThread::debugOpen(int reason, uint32_t data) {
...
void CoreThread::debug(bool state, int mode) {
    bool oldState;
    {
        std::lock_guard<std::mutex> lock(m_mutexDebug);
        oldState = m_debug;
...
void CoreThread::load(emu_data_t fileType, const QString &filePath) {
