...
- `coproc.present()` reports whether the Python ARM coprocessor exists. `coproc.bootloader()` returns `type`, `typeId`, and `description`, or `nil` when it is absent; known type strings are `"cemu-free"` and `"ti-uf2"`.
- `coproc.state()` returns a synchronized, read-only Cortex-M0+ snapshot, or `nil`. It contains `cycles`, `cycleLimit`, `sleeping`, general registers (with `sp`, `lr`, and `pc` aliases), the alternate stack pointer, active exceptions, execution flags, and SysTick, NVIC, and SCB state. `coproc.time()` returns just the ARM cycle count.
- `coproc.readByte/readHalf/readWord(address)` and matching `write*` functions perform synchronized CPU-style accesses to the ARM address space. Reads return `nil` and writes return `false` without a coprocessor. Accesses do not advance ARM cycles and may trigger the same peripheral side effects as firmware accesses.
- `coproc.reset()` resets or creates the coprocessor for a Python ASIC and reports whether one is present. `coproc.loadFlash(path)` loads an external ARM flash image for the current session and reports success; it does not change the configured override. The Qt external-flash caveat warning is shown after a successful load.
- `coproc.spiSelect(selected)`, `coproc.spiPeek()`, and `coproc.spiTransfer(value)` access the CE-facing SPI transport. `selected = true` asserts the active-low selection; peek and transfer return `{ bits, value }`, or `nil` without a coprocessor. `coproc.uartSend(byte)` reports whether the byte was queued, while `coproc.uartReceive()` consumes and returns one byte or `nil`. These low-level calls share the live transport with calculator software and can disrupt an active Python session.
